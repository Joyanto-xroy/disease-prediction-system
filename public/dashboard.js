    ...
    try {
        ...
        let query = supabase.from('patients').select(`
            *,
            ...
            assigned_doctor_profile:profiles!assigned_doctor(full_name)
        ...
        if (currentProfile && currentProfile.role === 'doctor' && currentUser && currentUser.id) {
            ...
            // Doctors see patients they created OR patients assigned to them
            query = query.or(`created_by.eq.${currentUser.id},assigned_doctor.eq.${currentUser.id}`);
        }
        ...
        el.innerHTML = buildTable(
            ['Name', 'DOB', 'Gender', 'Phone', ...(isAdmin ? ['Added By', 'Assigned Doctor'] : []), 'Actions'],
            data.map(p => [
                escHtml(p.full_name),
                ...
                escHtml(p.phone || 'N/A'),
                ...(isAdmin ? [
                    p.created_by_profile ? escHtml(p.created_by_profile.full_name) : 'N/A',
                    p.assigned_doctor_profile ? escHtml(`Dr. ${p.assigned_doctor_profile.full_name}`) : 'Unassigned'
                ] : []),
...
function openPatientModal(patientId = null) {
    ...
    // Show assigned doctor field only for admins
    const assignedDoctorGroup = document.getElementById('assignedDoctorGroup');
    const isAdmin = currentProfile && currentProfile.role === 'admin';
    if (assignedDoctorGroup) {
        assignedDoctorGroup.style.display = isAdmin ? 'block' : 'none';
    }
    ...
    if (patientId) {
        supabase.from('patients').select('*, profiles!assigned_doctor(full_name)').eq('id', patientId).single().then(({ data }) => {
            if (!data) return;
            ...
            if (document.getElementById('patientAssignedDoctor')) {
                document.getElementById('patientAssignedDoctor').value = data.assigned_doctor || '';
            }
        });
    }
...
async function loadDoctorsForAssignment() {
    try {
        ...
        const select = document.getElementById('patientAssignedDoctor');
        ...
        if (doctors && doctors.length > 0) {
            doctors.forEach(doctor => {
                ...
                option.textContent = `Dr. ${doctor.full_name}${doctor.specialization ? ` (${doctor.specialization})` : ''}`;
...
document.getElementById('patientForm').addEventListener('submit', async function (e) {
    ...
    const data = {
        ...
        assigned_doctor: (document.getElementById('patientAssignedDoctor') && document.getElementById('patientAssignedDoctor').value) || null,
    };
...
async function waitForService() {
    for (let i = 0; i < 5; i++) {
        const ok = await checkPredictionServiceHealth();
        if (ok) return true;
